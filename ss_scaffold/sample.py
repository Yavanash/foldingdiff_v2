...
def _load_training_means(model_dir: str, n_features: int) -> np.ndarray | None:
    """Means saved by ss_scaffold.train. Returns None if absent (model trained
    without zero_center, or pre-fix checkpoint)."""
    p = Path(model_dir) / "training_means.npy"
    if not p.exists():
        return None
    means = np.load(p).astype(np.float32)
    assert means.shape == (n_features,), (
        f"training_means.npy shape {means.shape} != ({n_features},)"
    )
    return means
...
def main():
    ...
    # The model is trained in mean-centered angle space (CathCanonicalAnglesDataset
    # zero_center=True). Both the motif clamp and the final output need to be
    # converted between raw and centered space.
    means = _load_training_means(args.model_dir, len(ANGLE_NAMES))
    if means is None:
        logging.warning(
            "training_means.npy not found in --model-dir. Sampling will operate "
            "in raw-angle space, which is mismatched with how the model was "
            "trained — non-motif residues will collapse. Re-train (or supply "
            "the means file) to fix."
        )
    else:
        logging.info(f"Loaded training means: {means}")
        motif_angles_np = _wrap_pi(motif_angles_np - means[None, :]).astype(np.float32)
    ...
    final = sampled[-1].numpy()  # (B, L, n_features), in mean-centered space

    # Shift back into raw angle space before reconstruction.
    if means is not None:
        final = _wrap_pi(final + means[None, None, :]).astype(np.float32)
