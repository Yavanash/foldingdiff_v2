...
def main():
    ...
    # Persist the angle-mean offset so sampling can round-trip between the
    # mean-centered space the model was trained in and raw angle space.
    means = train_dset.inner.get_masked_means()
    if means is not None:
        np.save(out_dir / "training_means.npy", means)
    train_loader = DataLoader(
        train_dset,
        batch_size=args.batch_size,
        shuffle=True,
